...
import { Send, Lightbulb, AlertCircle, Loader2, X } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white to-transparent dark:from-[#0F0F0F] dark:via-[#0F0F0F] dark:to-transparent pt-12">
      <div className="max-w-2xl mx-auto">
        ...
        {hintWord && (
          <div className="mb-4 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-900/30 rounded-2xl flex items-center gap-3 text-orange-600 dark:text-orange-400 text-sm font-bold animate-in fade-in slide-in-from-bottom-2">
            <Lightbulb className="w-5 h-5 flex-shrink-0" />
            <div className="flex-1">Hint: {hintWord}</div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="relative group">
          <input
            ...
            className={cn(
              "w-full py-6 pl-6 pr-32 rounded-3xl font-bold text-lg transition-all outline-none border-2",
              isDarkMode 
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
            <button
              type="button"
              onClick={handleHint}
              disabled={isValidating || hintLevel >= 2}
              className={cn(
